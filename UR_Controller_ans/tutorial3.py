...
def zero_velocity(point_list, duration_list, joint_num = 6):
    ...
    """
    speed_list = [np.zeros(joint_num)] * len(point_list)
    return speed_list
...
def average_velocity(point_list, duration_list, joint_num = 6):
    ...
    for i in range(1, N-1):
        ...
        speed2 = (point3 - point2) / duration2
        mid_speed = []

        # Check if velocities have the same direction (positive product means same direction)
        directions = speed1 * speed2
        for j in range(len(directions)):
            if directions[j] <= 0:
                # Different directions or zero: set velocity to zero to avoid oscillation
                mid_speed.append(0)
            else:
                # Same direction: use average of the two velocities for smooth transition
                mid_speed.append((speed1[j] + speed2[j]) / 2.0)
