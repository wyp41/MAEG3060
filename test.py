...
# T_i = np.array([
#             [np.cos(q[i]), -np.sin(q[i])*np.cos(alpha_standard[i]), np.sin(q[i])*np.sin(alpha_standard[i]), a_standard[i]*np.cos(q[i])],
#             [np.sin(q[i]), np.cos(q[i])*np.cos(alpha_standard[i]), -np.cos(q[i])*np.sin(alpha_standard[i]), a_standard[i]*np.sin(q[i])],
#             [0, np.sin(alpha_standard[i]), np.cos(alpha_standard[i]), d_standard[i]],
#             [0, 0, 0, 1]
#         ])
...
rtde_c = rtde_control.RTDEControlInterface("192.168.50.168")
rtde_r = rtde_receive.RTDEReceiveInterface("192.168.50.168")
...
for i in range(500):
    ...
    rtde_c.speedJ(q_dot.tolist(), acceleration, dt)
