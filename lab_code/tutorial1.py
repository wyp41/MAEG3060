...
def compute_cubic_coeffs(start, end, duration):
    ...
    targets = np.vstack([start, end, np.zeros_like(start), np.zeros_like(start)])
...
if __name__ == "__main__":
    ...
    start = np.array([90, -60, 60, -90, -90, 0])
    end = np.array([0, -90, 0, -90, 0, 0])
